...
namespace Drupal\workflow_wizard\Form;
 ...
 * workflow Form class.
 */
class workflowForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $options = NULL) {
    $teacher_id = \Drupal::request()->get('stage');
    //$teacher_details = \Drupal\user\Entity\User::load($teacher_id);
    $teacher_name = 'alex'; //alex_tweaks_get_real_name($teacher_details);
    //$teacher_details = alex_tweaks_get_extra_info($teacher_details);
    $teacher_mail = 'alexis.harper@ga.gov.au';// $teacher_details['mail'];
 
    $account_details = workflow_wizard_get_user_details();
  ...
  public function submitModalFormAjax(array $form, FormStateInterface $form_state) {
    ...
    if ($form_state->hasAnyErrors()) {
      ...
      \Drupal::logger('Workflow Wizard')->notice($message);
